    ...
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div>
          ...
          <SelectField items={locations}label={'اختر موضع القائمة'} register={{...register('location')}} />
          <h2 className="text-xl mt-5">عناصر القائمة</h2>
          {fields.map((item, index) => (
            <div key={item.id} className="mb-4">
              <TextField
                label="اسم العنصر"
                register={{...register(`items.${index}.title`, { required: true })}}
                error={errors.items?.[index]?.title?.message}
              />
              <TextField
                label="رابط العنصر"
                register={{...register(`items.${index}.url`, { required: true })}}
                error={errors.items?.[index]?.url?.message}
              />
              <button className='pb-5 text-red-500' type="button" onClick={() => remove(index)}>حذف العنصر</button>
            </div>
          ))}
        </div>
        <div className='ms-5 me-5 flex justify-between'>
        <TextButton className='m-5' ClickHandler={() => append({ title: '', url: '' })} text={'اضافة عنصر'} />
